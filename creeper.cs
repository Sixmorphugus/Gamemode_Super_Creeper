...
if(isFile("Add-Ons/System_ReturnToBlockland/server.cs")) {
    ...
    // who the fuck even checks that anymore
    
    // (the commented out settings are per-map now)
	$Creep::tickms = 20; //RTB_registerPref("Growth Speed","Creep Mod","Creep::tickms","list Normal 20 Slow 50 Slower 100 Slowest 200","GameMode_Super_Creeper",20,0,0,"CreepPrefOnChangeCreepSpeed");
	$Creep::wanderlust = -1; //RTB_registerPref("Player-Hunting","Creep Mod","Creep::wanderlust","list Laser-Guided 0 Merciless 1 Dogged 3 Pesky 9 Normal 19 Relaxed 49 Subtle 99 Off -1","GameMode_Super_Creeper",19,0,0,"CreepPrefOnChangeCreepHunting");
	RTB_registerPref("WaterFX Creeper","Creep Mod","Creep::undulo","bool","GameMode_Super_Creeper",1,0,0,0);
	RTB_registerPref("Creep Color","Creep Mod","Creep::Color","int 0 63","GameMode_Super_Creeper",7,0,0,0);
	...
	$Creep::HPFactor = 1; //RTB_registerPref("Creep HP","Creep Mod","Creep::HPSetting","list Juggernautical 1 Unyielding 2 Hardy 3 Normal 4 Weak 5 Fragile 6 Insta-Gib 7","GameMode_Super_Creeper",4,0,0,"CreepPrefOnChangeHPFactor");
    ...
    RTB_registerPref("Maximum HP Factor","Creep Mod","Creep::maxHPFac","int 0 100","GameMode_Super_Creeper",2,0,0,0);
	$Creep::regen = 1; //RTB_registerPref("Creep Regenerates","Creep Mod","Creep::regen","bool","GameMode_Super_Creeper",1,0,0,"CreepPrefOnChangeRegenerates");
	$Creep::regenms = 1000; //RTB_registerPref("Regeneration Speed","Creep Mod","Creep::regenms","list Insane 150 Faster 425 Fast 750 Normal 1000 Slow 1500 Slower 2000","GameMode_Super_Creeper",1000,0,0,"CreepPrefOnChangeRegenSpeed");
} else {
	$Creep::tickms = 20;
	$Creep::wanderlust = 19;
	$Creep::undulo = 1;
...
function GrowTick(%counter)
{
    ...
    if(CreepGroup.getCount() == 0) {
        $DefaultMiniGame.endRound($DefaultMiniGame.Teams.getTeamFromName("The Players"));
        // ^ bit of a hack but it works pretty well
    }
...
function SetCreepDmgColor(%brick)
{
	...
	%damlimit = (%damlimit + 5) * ($Creep::scalehp ? $DefaultMiniGame.playercount : 1) * $Creep::HPFactor;
...
function CreeperHit(%a, %b, %c, %d, %e, %f, %g) {
    if(!isobject(CreepGroup)) return;
	if(CreepGroup.ismember(%c) && $Creep::Active && $DefaultMiniGame == %b.sourceobject.client.minigame) {
		if(iseventpending(%c.regentick)) cancel(%c.regentick);
		%c.damage += %a.creepdamage;
		if(%c.justforced) {
			%c.justforced = 0;
		}
		%damlimit = ((15 - (CreepGroup.getcount()/1000)));
		...
		if(%c.damage >= %damlimit || $Creep::RoundType == 2) {
            if($Creep::RoundType != 3) {
                %c.delete();
                %b.sourceobject.client.kills++;
                UpgradeMan.incValue(%b.sourceobject.client.bl_id,"totalCreepKills",1);
			...
			SetCreepDmgColor(%c);
			if($Creep::regen) %c.regentick = scheduleNoQuota($Creep::regenms, 0, "BrickRegen", %c);
		}
	}
}
...
function StartCreeper()
{
    ...
    $DefaultMiniGame.playercount = %client.minigame.nummembers; // used for health scaling
